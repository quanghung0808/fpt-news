...
const AttractiveScreen = () => {
  const navigation = useNavigation();

  const [sortView, setSortView] = useState(false);
  let filterActractiveData = [];
  data.map((i) => {
    if (i.actractive) {
      filterActractiveData.push(i);
    }
  });
  const onClick = (value) => () => {
    setSortView(value);
  ...
  if (sortView) {
    filterActractiveData = [
      ...
      {
        title:
          "Take a look at the sustainable development activities of FPT Edu-er",
        description:
          "FPT Education students and staff across the nation have done several activities and made positive contributions to the community, economy, society, and ecosystem on a daily basis. Let’s take a look at three activities that are attracting FPT Education staff and students' attention.",
        content:
          "The Business hours is a series of career-oriented activities, which helps students improve their skills",
        img: "https://daihoc.fpt.edu.vn/en/wp-content/uploads/2022/08/279458421_3130764007145743_8954462282073093044_n-373x206.jpg",
        status: true,
        views: 341,
        actractive: true,
  ...
  return (
    <View className="flex-1" style={styles.background}>
      ...
      <TouchableOpacity onPress={onClick(!sortView)}>
        {sortView ? (
          <View style={styles.categoryClear}>
            <Text className="text-teal-200 mt-2 font-bold p-2 bg-lime-700 rounded-md text-center">
              Clear sorting
            </Text>
          </View>
        ) : (
          <View style={styles.category}>
            <Text className="text-teal-500 mt-2 font-bold p-2 bg-lime-200 rounded-md text-center">
              Sort by view
            </Text>
          </View>
        )}
...
export default AttractiveScreen;
